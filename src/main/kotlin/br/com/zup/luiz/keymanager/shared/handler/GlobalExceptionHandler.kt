package br.com.zup.luiz.keymanager.shared.handler
...
class GlobalExceptionHandler : ExceptionHandler<StatusRuntimeException, HttpResponse<Any>> {
    ...
    /*
     - Essa classe é gerenciada pelo micronaut e implementa ExceptionHandler
     ela espera dois tipos parametrizados 1 qual exception e o 2 qual tipo de resposta
     ela solicita a implementação do metodo handle
     */
    ...
    override fun handle(request: HttpRequest<*>, exception: StatusRuntimeException): HttpResponse<Any> {
        ...
        val (httpStatus, message) = when (statusCode) {
            ...
            Status.ALREADY_EXISTS.code -> Pair(HttpStatus.UNPROCESSABLE_ENTITY, statusDescription) //422 está aqui
            else ->  {
